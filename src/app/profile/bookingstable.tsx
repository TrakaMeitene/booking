                ...
                {data?.data?.map((item: any) => (
                    <div key={item.id} className="bg-white shadow rounded-lg p-4 mb-4">
                        <div className="grid grid-cols-2 gap-2">
                            <span className="font-semibold">Veikšanas datums:</span>
                            <span>{moment(item.created_at).format("DD.MM.yyyy HH:mm")}</span>

                            <span className="font-semibold">Pakalpojumu sniedzējs:</span>
                            <span>{item.specialist.name}</span>

                            <span className="font-semibold">Pakalpojums:</span>
                            <span>{item.service.name}</span>
                            <span className="font-semibold">Vizītes datums:</span>
                            <span>{moment(item.date).format("DD.MM.yyyy HH:mm")}</span>
                            <span className="font-semibold">Cena:</span>
                            <span>{item.service.price.toFixed(2)} Eur</span>
                            {user.scope === "all" && <span className="font-semibold">Statuss:</span>}
                            {user.scope === "all" &&<span>{item.statuss === "active" ? "Aktīva" : "Atteikta"}</span>}
